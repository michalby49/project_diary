...
class AdminController extends Controller {
    ...
    public function addEvent() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $eventData = [
                ...
                'user_id' => $_SESSION['user_id'] // Zakładamy, że użytkownik jest zalogowany
            ];
    ...
    public function editEvent($id) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $eventData = [
                ...
                'description' => $_POST['description'] ?? '',
                'category_id' => $_POST['category_id'] ?? null,
                'user_id' => $_SESSION['user_id'] // Pobieranie ID użytkownika z sesji (jeśli potrzebne)
            ];
    ...
    public function categories() {
        // Pobierz wszystkie kategorie i przekaż je do widoku
        $categoryModel = $this->model('Category');
        $categories = $categoryModel->getAllCategories();
    ...
    private function validateEvent($eventData){
        ...
        // Walidacja: Sprawdzanie, czy data zakończenia nie jest wcześniejsza niż data rozpoczęcia
